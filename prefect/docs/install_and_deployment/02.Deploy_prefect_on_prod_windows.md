...
There is an official doc [here](https://docs.prefect.io/v3/how-to-guides/self-hosted/server-windows). But I found it lack of many things.
...
### 5.1 Prefect configuration file(overrides env vars)

Prefect 3 supports configuration in YAML. 

For a specific user, you can create a file at: `%USERPROFILE%\.prefect\config.yaml`

For a system-wide config, you can create a file at: `C:\ProgramData\Prefect\config.yaml`

Example of the `config.yaml`

```yaml
api:
  database_connection_string: "postgresql+psycopg2://prefect:pass@localhost:5432/prefect_db"

logging:
  to_file: true
  log_directory: "D:\prefect_data\logs"

```

If you are not sure which are the current config value, you can use the below command to check
...
### 5.2 Start the prefect server manually
